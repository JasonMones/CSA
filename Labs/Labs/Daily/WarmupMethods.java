...
public class WarmupMethods {
    ...
    public int[] mergeIt(int[] array1, int[] array2) {
        ...
        for (int j = endIndex; j < array2.length+endIndex; j++) {
            mergedArray[j] = array2[j-endIndex];
        }
        Sorting s = new Sorting(mergedArray);
        return s.decreaseCapacity(s.sort());
    }
}
